...
5. **Command state resets in `setStart`** — instances are reused across runs.
   ...
   zero targets or reset state, never energize an actuator. Guard cleanup
   that must match a real run with a flag set in `setStart`.
...
- Helpers (`com.pedropathing.ivy.commands.Commands`): `instant {}`,
  `waitUntil {}`, `infinite {}`, `lazy {}`; for waits use
  `org.firstinspires.ftc.teamcode.core.util.monotonicWaitMs(ms)`, never Ivy's
